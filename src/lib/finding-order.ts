...
// Nacelle → Hub → Blade → Top section → Section A → B → C → D → Base → Outside.
...
const AREA_RULES: { rank: number; test: RegExp }[] = [
  ...
  { rank: 3, test: /top\s*section|section\s*top|\bđỉnh\b/i },
  { rank: 4, test: /section\s*a\b|khoang\s*a\b/i },
  { rank: 5, test: /section\s*b\b|khoang\s*b\b/i },
  { rank: 6, test: /section\s*c\b|khoang\s*c\b/i },
  { rank: 7, test: /section\s*d\b|khoang\s*d\b/i },
  { rank: 8, test: /\bbase\b|chân\s*(trụ|tháp)|móng/i },
  { rank: 9, test: /outside|bên\s*ngoài|ngoài/i },
];
